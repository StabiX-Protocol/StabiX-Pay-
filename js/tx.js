...
snap.forEach(doc => {
  ...
  // 🔽 filters same rakho
  if(window.filters.type && t.type !== window.filters.type) return;
  if(window.filters.asset && t.asset !== window.filters.asset) return;
...
list.forEach(t => {
  ...
  html += `
  <div onclick="openTxDetail('${id}')" style="
    display:flex;
  ...
  ">
    ...
    <div>
      <div style="font-weight:600;font-size:14px;">${userId}</div>
